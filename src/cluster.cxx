#include "sgwj/cluster.hxx"
#include "sgwj/functions.hxx"
#include <cassert>
#include <algorithm>
...
std::vector<double> Cluster::s_chebyshev_coefficients = SGWTFunctions::ChebyshevCoefficients(50);
...
void Cluster::SetInputs(std::vector<int> labels,
  ...
  for (int i=0; i<n_labels; i++){
    ...
    std::vector<double> pxpypx = PxPyPz(pts[i], rapidites[i], phis[i]);
    m_px.push_back(pxpypx[0]);
    m_py.push_back(pxpypx[1]);
    m_pz.push_back(pxpypx[2]);
  }
  ...
  m_distances = SGWTFunctions::NamedDistanceMatrix(m_pts, m_rapidites, m_phis,
                                                   SGWTFunctions::antikt);
  m_laplacian = SGWTFunctions::Laplacian(m_distances, m_sigma, normalised=true);
  // Check the max number of jets
  m_max_jets = m_n_rounds < n_labels ? m_n_rounds : n_labels;
  // Decide on the seed order
  summed_distances = std::vector<double>(n_labels, 0.);
  for (int i=0; i<n_labels; i++){
    for (int j=0; j<n_labels; j++){
      summed_distances[i] += m_distances[i][j];
    }
  }
  m_seed_order = std::vector<int>(n_labels);
  std::iota(m_seed_order.begin(), m_seed_order.end(), 0);
  std::sort(m_seed_order.begin(), m_seed_order.end(),
            [&summed_distances](int i1, int i2){return summed_distances[i1] < summed_distances[i2];});
};

const std::vector<const int>* Cluster::GetLabels(){
  return &m_labels;
};

const std::vector<const double>* Cluster::GetEnergies(){
  return &m_energies;
};

const std::vector<const double>* Cluster::GetEnergies(const std::vector<int>& labels){
...
const std::vector<const double>* Cluster::GetPts(){
  return &m_pts;
};

const std::vector<const double>* Cluster::GetPts(const std::vector<int>& labels){
  std::vector<const double> pts(labels.size());
  for (int label : labels){
...
const std::vector<const double>* Cluster::GetRapidites(){
  return &m_rapidites;
};

const std::vector<const double>* Cluster::GetRapidites(const std::vector<int>& labels){
  std::vector<const double> rapidites(labels.size());
  for (int label : labels){
...
const std::vector<const double>* Cluster::GetPhis(){
  return &m_phis;
};

const std::vector<const double>* Cluster::GetPhis(const std::vector<int>& labels){
  std::vector<const double> phis(labels.size());
  for (int label : labels){
...
std::vector<int> Cluster::GetNextMerge(){
  ...
  while (labels.size() == 0){
    ...
    std::vector<double> wavelets = SGWTFunctions::LaplacianWavelet(m_laplacian, s_chebychev_coefficients,
                                                                   seed, s_interval);
...
void Cluster::DoMerge(std::vector<int> labels, int label_new,
  ...
  std::vector<double> pxpypz = PxPyPz(pt, rapidity, phi);
...
std::vector<double> Cluster::GetMergedKinematics(std::vector<int> labels){
  ...
  std::vector<double> kinematics = Functions::PtRapPhi(px, py, pz);
